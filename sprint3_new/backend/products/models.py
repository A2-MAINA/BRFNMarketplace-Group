...
class Product(models.Model):
    ...
    # Shows a readable product label with its producer owner.
    def __str__(self):
...
class Allergen(models.Model):
    ...
    # Shows the allergen name in admin and related object displays.
    def __str__(self):
        return self.name
...
class Review(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['product', 'customer', 'order'], name='unique_review_per_order_product'),
        ]
        ordering = ['-created_at']


class AvailabilitySubscription(models.Model):
    customer = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='availability_subscriptions')
    product = models.ForeignKey('products.Product', on_delete=models.CASCADE, related_name='availability_subscriptions')
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['customer', 'product'], name='unique_product_availability_subscription'),
        ]
        ordering = ['-created_at']
...
class WholesalePrice(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['product', 'buyer_type'], name='unique_wholesale_price_per_buyer_type'),
        ]
        ordering = ['-updated_at']
